test_that("osrm_extract runs osrm-extract with expected arguments", {
  ...
  tmp_dir <- tempdir()
  input_osm <- file.path(tmp_dir, paste0("osrm-test-", Sys.getpid(), ".osm.pbf"))
  ...
  expected_timestamp <- paste0(sub("\\.osm\\.pbf$", "", normalized_input, ignore.case = TRUE), ".osrm.timestamp")

  captured <- list()
  mock_run <- function(command, args, echo, spinner, echo_cmd) {
    captured <<- list(command = command, args = args, echo = echo, spinner = spinner, echo_cmd = echo_cmd)
...
test_that("osrm_extract enforces overwrite flag for existing outputs", {
  ...
  tmp_dir <- tempdir()
  input_osm <- file.path(tmp_dir, paste0("osrm-overwrite-", Sys.getpid(), ".osm.pbf"))
  generated_prefix <- sub("\\.osm\\.pbf$", "", input_osm)
  on.exit({
    artifacts <- list.files(
      tmp_dir,
