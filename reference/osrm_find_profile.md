...
**\[stable\]** By default OSRM ships profiles for "car", "bike" and
"foot" in a `profiles/` directory alongside the binaries. This function
will try to locate `osrm-routed` on the `PATH`, resolve symlinks, and
look first for a `profiles/` directory next to the binary (as placed
there by
