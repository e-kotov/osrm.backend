...
read_pbf_header_bbox <- function(pbf_file) {
  if (!file.exists(pbf_file)) return(NULL)
  ...
  tryCatch({
    ...
    if (header_len <= 0 || header_len > 65536) return(NULL)  # Sanity check
    ...
    if (is.null(datasize) || datasize <= 0 || datasize > 33554432) return(NULL)
    ...
    while (pos <= length(blob_bytes)) {
      if (pos > length(blob_bytes)) break
    ...
    if (is.null(zlib_data)) return(NULL)
    ...
    if (is.null(decompressed)) return(NULL)
    ...
    while (pos <= length(decompressed)) {
      if (pos > length(decompressed)) break
      ...
      if (field_num == 1 && wire_type == 2) {
        # bbox message
        len <- .read_varint(decompressed, pos)$value
...
.parse_header_bbox <- function(bbox_bytes) {
  ...
  while (pos <= length(bbox_bytes)) {
    ...
    if (wire_type == 0) {  # varint
      ...
      if (field_num == 1) coords$left <- decoded
      else if (field_num == 2) coords$right <- decoded
      else if (field_num == 3) coords$top <- decoded
      else if (field_num == 4) coords$bottom <- decoded
    } else {
      break
    }
  }
...
.zlib_decompress <- function(data) {
  if (length(data) == 0) stop("Empty data to decompress")
  ...
  if (nzchar(zf)) {
    ...
    exit_code <- system2(zf, args = c("-uncompress"), 
                         stdin = temp_in, stdout = temp_out, stderr = FALSE)
  ...
  if (nzchar(py)) {
    ...
    cat("import sys, zlib; open(sys.argv[1],'wb').write(zlib.decompress(open(sys.argv[2],'rb').read()))",
        file = py_script)
    writeBin(data, temp_in)
    
    exit_code <- system2(py, args = c(shQuote(py_script), shQuote(temp_out), shQuote(temp_in)))
    if (exit_code == 0 && file.exists(temp_out) && file.size(temp_out) > 0) {
      return(readBin(temp_out, "raw", n = file.size(temp_out)))
    }
  }
  ...
  result <- tryCatch({
    if (length(data) > 6 && data[1] == as.raw(0x78) &&
...
.get_pbf_extent <- function(pbf_file) {
  ...
  if (nzchar(osmium_path)) {
    tryCatch({
      info <- system2(osmium_path, args = c("fileinfo", shQuote(pbf_file)),
